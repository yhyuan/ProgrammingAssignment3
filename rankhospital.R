rankhospital <- function(state, outcome, num = "best") {
	...
	if (!(state %in% unique(dt$State))) {
		...
		#dt.subset$Rank <- rank(dt.subset$Rate)
		dt.subset <- dt.subset[order(dt.subset[,3], dt.subset[,1],decreasing=FALSE),]
		dt.subset$Rank <- c(1:nrow(dt.subset))
		Rank <- num
		if (num == "best") {
			Rank <- 1
		}
		if (num == "worst") {
			Rank <- max(dt.subset[!is.na(dt.subset$Rate), ]$Rank)
		}		
		dt.subset <- dt.subset[dt.subset$Rank == Rank, ]
		if(nrow(dt.subset) == 0) {
			c(NA)
		} else {
